  ...
  return (
    <div className="border border-gray-300 rounded p-2 bg-white">
      <div className="flex flex-wrap gap-2 mb-2">
        {tags.map((tag, index) => (
          <div key={index} className="bg-teal-500 text-white px-3 py-1 rounded-full flex items-center text-sm">
            {tag}
            <button type="button" onClick={() => removeTag(index)} className="ml-2 focus:outline-none">
              <RemoveIcon />
            </button>
          </div>
        ))}
        ...
        <main className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
          ...
          <Formik
            ...
            onSubmit={async (values, { setSubmitting }) => {
              try {
                if (!thumbnailFile) {
                  throw new Error('Please upload the course thumbnail to proceed.');
                }
                  ...
                  {({ push: pushModule, remove: removeModule }) => (
                    <div className="space-y-6">
                      {values.modules.map((module, moduleIndex) => {
                        const moduleErrors = (errors.modules?.[moduleIndex] as any) || {};
                        const moduleTouched = touched.modules?.[moduleIndex] || {};
                        const isEditingLesson = module.lessons.some((lesson) => lesson.isEditing);

                        return (
                          <div key={moduleIndex} className="bg-gray-50 shadow-md rounded-lg overflow-hidden">
                            <div className="border-b border-gray-200 bg-gray-100">
                              ...
                              >
                                <div className="flex items-center">
                                  <h2 className="text-xl font-semibold text-gray-900">Module {moduleIndex + 1}</h2>
                                  {module.moduleTitle && (
                                    <span className="ml-3 text-gray-500">- {module.moduleTitle}</span>
                                  )}
                                </div>
                                <div className="flex items-center">
                                  <svg
                                ...
                                <div className="mb-6">
                                  ...
                                  <Field
                                    ...
                                    className={`mt-1 block w-full rounded-md shadow-sm py-3 px-4 border ${
                                      moduleErrors.moduleTitle && moduleTouched.moduleTitle
                                  ...
                                  {({ push: pushLesson, remove: removeLesson }) => (
                                    <div className="space-y-6">
                                      {module.lessons.map((lesson, lessonIndex) => {
                                        const lessonErrors = (moduleErrors.lessons?.[lessonIndex] as any) || {};
                                        const lessonTouched = moduleTouched.lessons?.[lessonIndex] || {};
                                        const isLessonValid =
                                              ...
                                              <button
                                                ...
                                                onClick={() => {
                                                  ...
                                                  newModules[moduleIndex].lessons[lessonIndex] = {
                                                    ...newModules[moduleIndex].lessons[lessonIndex],
                                                    isOpen: !newModules[moduleIndex].lessons[lessonIndex].isOpen,
                                                  };
                                                  setFieldValue('modules', newModules);
                                                }}
                                              >
                                                <div className="flex items-center">
                                                  ...
                                                  {lesson.title && (
                                                    <span className="ml-3 text-gray-500">- {lesson.title}</span>
                                                  )}
                                                </div>
                                                <div className="flex items-center">
                                                  {lesson.uploaded && (
                                                ...
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                                  <div className="space-y-4">
                                                    <VideoFileInput
                                                      ...
                                                      initialValue={{
                                                        file: lesson.video,
                                                        duration: parseInt(lesson.duration || '0', 10),
                                                      }}
                                                    />
                                                    <ErrorMessage
                                                  ...
                                                  <div className="space-y-4">
                                                    <div>
                                                      ...
                                                      <Field
                                                        ...
                                                        className={`mt-1 block w-full rounded-md shadow-sm py-3 px-4 border ${
                                                          lessonErrors.title && lessonTouched.title
                                                            ? 'border-red-500 focus:border-red-500 focus:ring-red-500'
                                                            : 'border-gray-300 focus:border-teal-500 focus:ring-teal-500'
                                                        }`}
                                                      />
                                                    ...
                                                    <div>
                                                      ...
                                                      <Field
                                                        ...
                                                        className={`mt-1 block w-full rounded-md shadow-sm py-3 px-4 border ${
                                                          lessonErrors.description && lessonTouched.description
                                                            ? 'border-red-500 focus:border-red-500 focus:ring-red-500'
                                                            : 'border-gray-300 focus:border-teal-500 focus:ring-teal-500'
                                                        }`}
                                                      />
                                                    ...
                                                    <button
                                                      ...
                                                      onClick={() => {
                                                        ...
                                                        toast.success(`Lesson ${lesson.lessonNumber} saved successfully!`, {
                                                          position: "top-right",
                                                          autoClose: 3000,
                                                        });
                                                      }}
                                                    ...
                                                    >
                                                      <CheckIcon />
                                                      <span className="ml-2">Save Lesson</span>
                                                    </button>
                                                  </div>
                                                </div>
                                              </div>
                                      ...
                                      <div className="flex justify-between pt-4">
                                        <div className="flex space-x-3">
                                          ...
                                          {module.lessons.length > 1 && (
                                            <button
                                              ...
                                              onClick={() => {
                                                ...
                                                if (!lastLesson.isEditing) {
                                                  removeLesson(module.lessons.length - 1);
                                                }
                                              }}
                                            ...
                                            >
                                              <RemoveIcon />
                                              <span className="ml-2">Remove Last Lesson</span>
                                            </button>
                                          )}
                                        </div>
                                      </div>
                      ...
                      <div className="flex justify-between pt-4">
                        <div className="flex space-x-3">
                          ...
                          {values.modules.length > 1 && (
                            <button
                              type="button"
                              onClick={() => removeModule(values.modules.length - 1)}
                        ...
                        <button
                          ...
                          className={`inline-flex items-center px-6 py-3 border border-transparent rounded-md shadow-sm text-sm font-medium text-white ${
                            isSubmitting ? 'bg-gray-400 cursor-not-allowed' : 'bg-teal-500 hover:bg-teal-600'
                          } focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-teal-500`}
                        >
